...
async function seed() {
  console.log("Fetching all existing reviews to delete them one by one...");
  
  let hasMore = true;
  let totalDeleted = 0;
  
  while (hasMore) {
    const { data: oldReviews, error: fetchReviewsError } = await supabase
      .from('reviews')
      .select('id')
      .limit(1000);
      
    if (fetchReviewsError) {
      console.error("Error fetching old reviews:", fetchReviewsError);
      break;
    }
    
    if (oldReviews && oldReviews.length > 0) {
      console.log(`Found ${oldReviews.length} old reviews in this batch. Deleting one by one...`);
      
      let deletedInBatch = 0;
      for (const r of oldReviews) {
        const { error: deleteError } = await supabase.from('reviews').delete().eq('id', r.id);
        if (deleteError) {
          console.error(`Error deleting review ${r.id}:`, deleteError);
        } else {
          deletedInBatch++;
          totalDeleted++;
        }
      }
      console.log(`Deleted ${deletedInBatch} in this batch. Total so far: ${totalDeleted}`);
      
      // If we couldn't delete any in this batch (maybe RLS issue), we must break to avoid infinite loop
      if (deletedInBatch === 0) {
        console.error("Could not delete any reviews in this batch. Stopping deletion loop.");
        break;
      }
    } else {
      hasMore = false;
  ...
  console.log(`Finished deleting. Total reviews deleted: ${totalDeleted}`);
  ...
  for (let i = 0; i < newReviews.length; i += chunkSize) {
    ...
    if (insertError) {
      ...
      console.log(`Inserted ${i + chunk.length} / ${newReviews.length}`);
    }
  }
