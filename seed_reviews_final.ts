...
async function seed() {
  console.log("Fetching all existing reviews to delete them properly (Handling Supabase 1000 limit)...");
  ...
  while (hasMore) {
    ...
    if (oldReviews && oldReviews.length > 0) {
      const idsToDelete = oldReviews.map(r => r.id);
      console.log(`Found ${oldReviews.length} old reviews in this batch. Deleting...`);
      
      const chunkSize = 100;
      for (let i = 0; i < idsToDelete.length; i += chunkSize) {
        const chunk = idsToDelete.slice(i, i + chunkSize);
        const { error: deleteError } = await supabase.from('reviews').delete().in('id', chunk);
        if (deleteError) {
  ...
  // Distribution to ensure more 4.8 and 4.9 than 5.0
  // 0 = 5.0 avg
  // 1 = 4.9 avg
  // 2 = 4.9 avg (lower end)
  // 3 = 4.8 avg
  const numFourStarOptions = [
    0, // 5.0 (1 chance)
    1, 1, 2, 2, // 4.9 (4 chances)
    3, 3, 3 // 4.8 (3 chances)
  ];

  for (const bike of bikes) {
    // Generate exactly 15 reviews per bike
    const numReviews = 15;
    
    // Pick the number of 4-star reviews
